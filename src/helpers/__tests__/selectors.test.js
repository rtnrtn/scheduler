...
test("getInterviewersForDay returns an array", () => {
  ...
  expect(Array.isArray(result)).toBe(true); // mon has int # 2
});

test("getInterviewersForDay returns an array with a length matching the number of interviewers for that day", () => {
  const result = getInterviewersForDay(state, "Monday");
  expect(result.length).toEqual(1); // mon has 1 interviewer
});

test("getInterviewersForDay returns an array containing the correct interviewer objects", () => {
  const interviews = getInterviewersForDay(state, "Tuesday");
  expect(interviews).toContain(state.interviewers["2"]);
});
